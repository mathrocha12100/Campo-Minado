const createBoard = (rows, colums) => { // cria uma matriz 
    return Array(rows).fill(0).map((_, row) => {
        return Array(colums).fill(0).map((_, column) => {
            return {
                row, //matriz linha
                column, //matriz coluna
                opened: false, // se esta aberto
                flagged: false, // se está flagged
                mined: false, // se foi mined
                exploded: false, // se explodiu
...
const spreadMines = (board, minesAmount) => { // função para espalhar as minas
